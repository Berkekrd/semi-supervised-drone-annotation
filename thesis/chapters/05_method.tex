...
Datenassoziation kombiniert. Eine Besonderheit von ByteTrack besteht darin, dass
nicht nur Detektionen mit hoher Konfidenz, sondern auch solche mit niedriger
Konfidenz in die Datenassoziation einbezogen werden. Dadurch können kurzzeitig
unsicher erkannte Objekte über aufeinanderfolgende Frames hinweg dennoch stabil
verfolgt werden, was zu robusteren Objekttrajektorien führt.
...
Die tracking-basierte Label-Propagation wurde in dieser Arbeit nicht als
zusätzlicher Retraining-Schritt eingesetzt. Ziel des Trackings war vielmehr die
Untersuchung, ob automatisch erkannte Objekte über aufeinanderfolgende Frames
hinweg konsistent verfolgt werden können und somit die zeitliche Konsistenz der
Annotationen verbessert wird. Die Bewertung erfolgt daher bewusst qualitativ
anhand repräsentativer Videosequenzen und der erzeugten Tracking-Ergebnisse.
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        \hline
        Frame-Ebene               & Quantitativ                 & Precision, Recall, mAP@50            \\
        Track-Ebene               & Qualitativ                  & Zeitliche Konsistenz der Detektionen \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{Iterationen des Self-Training-Prozesses und ihr erwarteter Beitrag}
    \label{tab:iteration_comparison}
    \begin{tabular}{p{2.2cm}p{4.6cm}p{6.2cm}}
        \hline
        \textbf{Iteration} & \textbf{Trainingsdaten}           & \textbf{Erwarteter Beitrag}           \\
        \hline
        Iteration 0        & Manuelle Annotationen             & Referenzleistung des Baseline-Modells \\
        Iteration 1        & + Pseudo-Labels                   & Erweiterte Klassenabdeckung           \\
        Iteration 2        & + Tracking-basierte Pseudo-Labels & Zeitliche Stabilisierung der Labels   \\
        \hline
    \end{tabular}
\end{table}
